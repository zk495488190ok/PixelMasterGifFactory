...
def conv2d(x, input_filters, output_filters, kernel, strides, mode='REFLECT'):
    with tf.compat.v1.variable_scope('conv'):
        ...
        weight = tf.Variable(tf.random.truncated_normal(shape, stddev=0.1), name='weight')
...
def conv2d_transpose(x, input_filters, output_filters, kernel, strides):
    with tf.compat.v1.variable_scope('conv_transpose'):
...
def instance_norm(x):
    ...
    return tf.math.divide(tf.subtract(x, mean), tf.sqrt(tf.add(var, epsilon)))
